...
describe("Clearance flow", () => {
  beforeEach(() => {
    cy.intercept("GET", "/api/clearance/", {
      fixture: "clearance_empty.json",
    }).as("list");
    cy.visit("/clearance");
    cy.wait("@list");
  });
  ...
  it("uploads, previews, downloads, edits and deletes a clearance document", () => {
    // stub upload
    cy.intercept("POST", "/api/clearance/", {
      id: 123,
      title: "Test Doc",
    ...
    // stub download blob
    cy.intercept("GET", "/api/clearance/123/download/", {
      fixture: "test.pdf",
    }).as("download");
    ...
    // Edit metadata
    cy.intercept("PATCH", "/api/clearance/123/", {
      id: 123,
      title: "Test Doc v2",
      department: "Dept",
    }).as("patch");
    ...
    // Delete
    cy.intercept("DELETE", "/api/clearance/123/", { statusCode: 204 }).as(
      "del",
    );
