...
jest.mock('@tanstack/react-query', () => {
  ...
  function useQuery(key, fn) {
    ...
    try {
      const res = fn()
      if (res && typeof res.then === 'function') return { data: undefined, isLoading: true }
      store.set(k, res)
      return { data: res, isLoading: false }
    } catch (e) { return { data: undefined, isLoading: false } }
  }
  ...
  return { useQuery, useMutation, useQueryClient, QueryClient, QueryClientProvider }
})
...
describe('AiSupportPage', () => {
  ...
  api.createConversation.mockImplementation(() => ({ id: 1, message: 'hey', response: 'hello' }))
  api.createStudyRecommendation.mockImplementation(() => ({ id: 1, recommendation: 'Study more' }))
