...
function FileDrop({ onChange, maxSizeMB = 10, allowedTypes = null }) {
  ...
  const validate = (file) => {
    ...
    if (allowedTypes && allowedTypes.length) {
      const ok = allowedTypes.some(
        (t) =>
          file.type === t ||
          file.name.toLowerCase().endsWith(t.replace("application/", ".")),
      );
...
export default function NewRequestModal({ isOpen, onClose }) {
        ...
        for (let i = 0; i < files.length; i++) {
          ...
          try {
            await uploadLeaveDocument(data.id, fd, (ev) => {
              ...
              const loadedSum = Object.values(perFileLoaded).reduce(
                (a, b) => a + b,
                0,
              );
              setBytesLoadedTotal(loadedSum);
              const pct = totalBytes
                ? Math.round((loadedSum / totalBytes) * 100)
                : 0;
              setCombinedProgress(pct);
              const filePct = ev.total
  ...
  const submit = async (e) => {
    ...
    if (step === 1) {
      ...
      if (!anyGood) {
        ferr.files =
          "Please attach at least one supporting document or proceed without files.";
      }
      setFileErrors(ferr);
  ...
  const retryUpload = async (requestId, index) => {
    ...
    try {
      ...
      await uploadLeaveDocument(rid, fd, (ev) => {
        ...
        const totalBytes = files.reduce((s, e) => s + (e.file?.size || 0), 0);
        const combined = totalBytes
          ? Math.round(
              (ev.loaded / (files[index].file.size || 1)) *
                (files[index].file.size / totalBytes) *
                100,
            )
          : pct;
      ...
      >
        <div className="min-h-screen px-4 text-center">
          ...
          <motion.div
            ...
            className="inline-block w-full max-w-xl p-6 my-8 overflow-hidden align-middle transition-all transform bg-white dark:bg-gray-900 shadow-xl rounded-2xl"
          >
            ...
            <div className="mt-3 mb-4">
              <div className="flex items-center space-x-3">
                <div className={`text-xs ${step === 0 ? "font-semibold" : ""}`}>
                  1. Details
                </div>
                <div className={`text-xs ${step === 1 ? "font-semibold" : ""}`}>
                  2. Documents
                </div>
                <div className={`text-xs ${step === 2 ? "font-semibold" : ""}`}>
                  3. Review
                </div>
              </div>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <select
                      ...
                      className="mt-1 block w-full rounded-md border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800"
                    >
              ...
              {step === 1 && (
                <div className="space-y-4">
                  ...
                  <div className="text-sm text-gray-500">
                    You can add multiple documents. After submission, files will
                    be attached to your request.
                  </div>
                      ...
                      {files.map((fobj, idx) => (
                        ...
                        >
                          <div>
                            ...
                            <div className="text-xs text-gray-500">
                              {Math.round(fobj.file.size / 1024)} KB{" "}
                              {fobj.error ? (
              ...
              {step === 2 && (
                <div className="space-y-4">
                  ...
                  <div className="p-3 border rounded">
                    <div>
                      <strong>Type:</strong> {form.leave_type}
                    </div>
                    <div>
                      <strong>Dates:</strong> {form.start_date} →{" "}
                      {form.end_date}
                    </div>
                    <div className="mt-2">
                      <strong>Reason:</strong>
                      <div className="text-sm mt-1">{form.reason}</div>
                    </div>
                    <div className="mt-2">
                  ...
                  <div className="text-sm text-gray-500">
                    When you submit, your request and its documents will be
                    uploaded.
                  </div>
                                ...
                                {fileErrors[idx] ? (
                                  <>
                                    <button
                                      ...
                                      onClick={() =>
                                        retryUpload(
                                          /*requestId set after submit*/ null,
                                          idx,
                                        )
                                      }
                                      className="text-xs text-indigo-600 mr-2"
                                    >
                                      Retry
                                    </button>
                                  </>
                  ...
                  {mutation.isLoading && (
                    <div className="mt-3">
                      <div className="text-xs text-gray-500 mb-1">
                        Uploading documents: {combinedProgress}%
                      </div>
                      <div className="w-full h-2 bg-gray-100 rounded overflow-hidden">
                        <div
                          className="h-2 bg-indigo-600"
              ...
              <div className="flex justify-between mt-6">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <button
                    ...
                    className="px-4 py-2 rounded-md bg-gradient-to-r from-indigo-600 to-indigo-400 text-white flex items-center space-x-2"
                  >
                    {mutation.isLoading ? (
                      <svg
      ...
      {(() => {
        ...
        if (uploadStartTs && bytesLoadedTotal > 0) {
          ...
          const remaining = Math.max(
            0,
            files.reduce((s, e) => s + (e.file?.size || 0), 0) -
              bytesLoadedTotal,
          );
