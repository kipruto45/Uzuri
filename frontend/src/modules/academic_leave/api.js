...
export const getLeaveRequest = (id) => axiosClient.get(`/academic-leave-requests/${id}/`).then(r => r.data)
export const createLeaveRequest = (payload) => axiosClient.post('/academic-leave-requests/', payload).then(r => r.data)
export const uploadLeaveDocument = (id, formData, onUploadProgress) => axiosClient.post(`/academic-leave-requests/${id}/submit_document/`, formData, { headers: { 'Content-Type': 'multipart/form-data' }, onUploadProgress }).then(r => r.data)
...
export const fetchLeaveStatus = (params) => axiosClient.get('academic_leave/status/', { params }).then(r => r.data)
