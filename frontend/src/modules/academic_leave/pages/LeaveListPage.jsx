...
export default function LeaveListPage() {
  ...
  const { data, isLoading } = useQuery(["leaveRequests"], () =>
    listLeaveRequests({ page_size: 20 }),
  );
        ...
        {data.results?.map((l) => (
          <div key={l.id} className="bg-white rounded shadow p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <div
                className={`text-xs px-2 py-1 rounded ${l.status === "approved" ? "bg-green-100 text-green-800" : l.status === "rejected" ? "bg-red-100 text-red-800" : "bg-yellow-100 text-yellow-800"}`}
              >
                {l.status}
              </div>
            </div>
