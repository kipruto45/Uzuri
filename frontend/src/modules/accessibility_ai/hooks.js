...
import { listAccessibilityFeatures, createAccessibilityFeature, toggleAccessibilityFeature } from './api'

export const ACCESSIBILITY_KEY = ['accessibility', 'features']

export function useAccessibilityFeatures() {
  return useQuery(ACCESSIBILITY_KEY, listAccessibilityFeatures, { staleTime: 1000 * 60 })
}
...
export function useCreateFeature() {
  ...
  return useMutation(createAccessibilityFeature, {
    onMutate: async (newFeature) => {
      ...
      qc.setQueryData(ACCESSIBILITY_KEY, (old = []) => [{ id: 'temp-' + Date.now(), ...newFeature }, ...old])
      return { previous }
    },
...
export function useToggleFeature() {
  ...
  return useMutation(({ id, enabled }) => toggleAccessibilityFeature(id, { enabled }), {
    onMutate: async ({ id, enabled }) => {
      await qc.cancelQueries(ACCESSIBILITY_KEY)
      const previous = qc.getQueryData(ACCESSIBILITY_KEY)
      qc.setQueryData(ACCESSIBILITY_KEY, (old = []) => old.map((f) => (f.id === id ? { ...f, enabled } : f)))
      return { previous }
    },
