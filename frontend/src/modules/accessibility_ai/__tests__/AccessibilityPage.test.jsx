...
jest.mock('../api', () => ({
  ...
  listAccessibilityFeatures: jest.fn(() => [{ id: 1, feature_type: 'high_contrast', enabled: true }]),
  toggleAccessibilityFeature: jest.fn((id, payload) => ({ id, feature_type: 'high_contrast', enabled: payload.enabled })),
  createAccessibilityFeature: jest.fn((p) => ({ id: 2, feature_type: p.feature_type, enabled: p.enabled })),
}))
...
describe('AccessibilityPage', () => {
  test('renders features and allows toggle and create', async () => {
    ...
    expect(await screen.findByText(/Accessibility features/i)).toBeInTheDocument()
    ...
    await userEvent.type(screen.getByPlaceholderText(/New feature type/i), 'spoken_feedback')
