...
export default function FileCard({ file, onDelete, onDownload }) {
  ...
  return (
    <>
        ...
        <div className="w-full h-40 bg-gray-50 flex items-center justify-center">
          {file.thumbnail_url ? (
            <button onClick={() => setOpen(true)} className="w-full h-full">
              <img src={file.thumbnail_url} alt={file.alt_text || file.name} className="object-contain h-full w-full" />
            </button>
          ) : (
      ...
      <Lightbox open={open} src={file.thumbnail_url} alt={file.alt_text || file.name} onClose={() => setOpen(false)} />
    </>
  )
}
