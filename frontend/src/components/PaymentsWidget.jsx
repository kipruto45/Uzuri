...
export default function PaymentsWidget() {
  ...
  if (isError) return (
    <div className="bg-white border rounded p-4">
      ...
      <div className="text-sm text-red-600 mt-2">Unable to load payments <button onClick={() => refetch()} className="ml-2 text-blue-600">Retry</button></div>
    </div>
  )
      ...
      <CardContent>
        ...
        <div className="mt-2 text-sm">
          ...
          <ul className="mt-1 list-disc pl-5 text-sm">
            {recent.map((p) => (
              <li key={p.id}>{p.description || p.reference} — {p.amount}</li>
            ))}
          </ul>
          <div className="mt-3">
            <button onClick={handleQuickPay} disabled={createMut.isLoading} className="px-3 py-1 rounded bg-green-600 text-white disabled:opacity-50">{createMut.isLoading ? 'Initiating…' : 'Quick pay'}</button>
          </div>
        </div>
      </CardContent>
    </Card>
