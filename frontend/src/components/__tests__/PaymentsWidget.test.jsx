      ...
      if (Array.isArray(key) && key[0] === 'payments') {
        try {
          ...
          if (axios.get && axios.get.mock && axios.get.mock.results[0]) {
            ...
            if (res && res.data) return { data: res.data, isSuccess: true, isLoading: false }
          }
        } catch (e) {}
        return { data: { next_due: null, payment_history: [] }, isSuccess: true, isLoading: false }
      }
...
jest.mock('../../hooks/usePayments', () => ({
  usePaymentsSummary: () => ({ data: { next_due: { due_date: '2025-10-03', amount: 100 }, payment_history: [{ id: 1, amount: 100, reference: 'abc' }] }, isLoading: false, isError: false }),
  useCreatePayment: () => ({ isLoading: false, mutateAsync: jest.fn().mockResolvedValue({}) }),
}))
...
test('PaymentsWidget shows recent payments', async () => {
  axios.get.mockResolvedValue({ data: { next_due: { due_date: '2025-10-03', amount: 100 }, payment_history: [{ id: 1, amount: 100, reference: 'abc' }] } })
