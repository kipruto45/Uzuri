  ...
  useEffect(() => {
    ...
    if (stored) {
      try {
        ...
        if (tokens?.access) axiosClient.defaults.headers.common.Authorization = `Bearer ${tokens.access}`
        api.me().then((u) => setUser(u)).catch(() => setUser(null)).finally(() => setLoading(false))
      } catch (e) {
        setLoading(false)
      }
    } else {
