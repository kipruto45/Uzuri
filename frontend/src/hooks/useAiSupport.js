...
export function useConversations(queryOptions = {}) {
  const q = useQuery(["ai-support", "conversations"], listConversations, {
    staleTime: 1000 * 20,
    ...queryOptions,
  });
  ...
  useEffect(() => {
    ...
    const WS_BASE =
      process.env.VITE_WS_BASE || process.env.REACT_APP_WS_BASE || "";
    if (!WS_BASE) return undefined;
    let ws;
    try {
      ws = new WebSocket(
        WS_BASE.replace(/^http/, "ws") + "/ai-support/updates/",
      );
      ws.onopen = () => {
        // subscribe or log
      };
...
export function useCreateConversation() {
  ...
  return useMutation((payload) => createConversation(payload), {
    ...
    onError: (err, newConv, context) =>
      qc.setQueryData(["ai-support", "conversations"], context.previous),
    onSettled: () => qc.invalidateQueries(["ai-support", "conversations"]),
  });
}

export function useStudyRecommendations() {
  return useQuery(["ai-support", "recommendations"], listStudyRecommendations, {
    staleTime: 1000 * 60,
  });
}
...
export function useAlerts(queryOptions = {}) {
  return useQuery(["ai-support", "alerts"], listAlerts, {
    staleTime: 1000 * 20,
    ...queryOptions,
  });
}
...
export function useDeleteAlert() {
  ...
  return useMutation((id) => deleteAlert(id), {
    onMutate: async (id) => {
      ...
      qc.setQueryData(["ai-support", "alerts"], (old = []) =>
        (old || []).filter((a) => a.id !== id),
      );
      return { previous };
    },
    onError: (err, id, context) =>
      qc.setQueryData(["ai-support", "alerts"], context.previous),
