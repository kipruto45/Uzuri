      ...
      if (Array.isArray(key) && key[0] === 'notifications') {
        ...
        try {
          ...
          if (svc.listNotifications) {
            const data = svc.listNotifications(key && key[1] && key[1].limit ? { limit: key[1].limit } : undefined)
...
test('useNotifications fetches list', async () => {
  jest.spyOn(notifications, 'listNotifications').mockImplementation(() => [{ id: 1, title: 'Hello' }])
