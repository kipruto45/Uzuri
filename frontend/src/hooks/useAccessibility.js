...
} from "../modules/accessibility_ai/api";

export function useAccessibilityFeatures() {
  return useQuery(["accessibility", "features"], listAccessibilityFeatures, {
    staleTime: 1000 * 30,
  });
}
  ...
  return useMutation(
    ({ id, enabled }) => toggleAccessibilityFeature(id, { enabled }),
    {
      onMutate: async ({ id, enabled }) => {
        ...
        qc.setQueryData(["accessibility", "features"], (old) =>
          old.map((f) => (f.id === id ? { ...f, enabled } : f)),
        );
        return { previous };
      },
      onError: (err, variables, context) =>
        qc.setQueryData(["accessibility", "features"], context.previous),
